...
OOS_quarters <- c("2021_Q2","2022_Q1","2022_Q3","2023_Q3","2023_Q4")
```
...
for (selected_quarter in OOS_quarters) {
  ...
  data.group2.oos.return <- 10000*diff.xts(log(data.group2.oos))
  names(data.group2.oos.return)[1:2] <- c("XAG.return","XAU.return")
  
  data.group2.oos <- merge(data.group2.oos[, c("XAG.close", "XAU.close")],
                       data.group2.oos.return[, c("XAG.return", "XAU.return")])
  ...
  print(chart_Series(data.group2.oos$XAG.return, theme = myTheme))
  print(chart_Series(data.group2.oos$XAU.return, theme = myTheme))
  layout(matrix(1))
  ...
  cmd.sds.ratio <- period.apply(data.group2.oos,
                               INDEX = indices,
                               function(x) sd(x$XAU.return, na.rm = TRUE) /
                                 sd(x$XAG.return, na.rm = TRUE)
  )
  ...
  data.group2b.oos$av.ratio <- na.locf(data.group2b.oos$av.ratio,
                               na.rm = FALSE)
  data.group2b.oos$sds.ratio <- na.locf(data.group2b.oos$sds.ratio,
                                na.rm = FALSE)
  ...
  data.group2b.oos$spread_sdsratio <- 
    data.group2b.oos$XAU.return - 
    data.group2b.oos$sds.ratio * data.group2b.oos$XAG.return
  ...
  for(volat.sd in c(180)) { # different volatility memories
    for(m_ in c(1)) { # different multipliers
      ...
      signal2 <- coredata(data.group2b.oos$spread_sdsratio)
      ...
      gross.pnl2 <- (pos.mr2) *
        (diff.xts(XAU_price) * 100  # point value for XAU
         - coredata(data.group2b.oos$sds.ratio) * diff.xts(XAG_price) * 5000) # point value for XAG
      ...
      net.pnl2 <- gross.pnl2 - ntrans2 * (12 + coredata(data.group2b.oos$sds.ratio) * 7)
...
```{r, warning = F, message = F}
options(digits=2)
quarter_stats.all.group1.oos
```

```{r, warning = F, message = F}
options(digits=2)
quarter_stats.all.group2.oos
```
...
```{r, warning = F, message = F}
# write.csv(quarter_stats.all.group1.oos, 
#           "quarter_stats.all.group1.oos.csv",
#           row.names = FALSE)1

# write.csv(quarter_stats.all.group2.oos, 
#           "quarter_stats.all.group2.oos.csv",
#           row.names = FALSE)
```
