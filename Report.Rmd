...
For the `Quantitative Strategies on High Frequency Data` project, two strategies were developed for two groups of assets.
...
For group 2, futures contracts on gold and silver were considered. Multiple combinations within level-based and return-based pair trading strategies were tested. Similarly, the best variant was chosen based on in-sample data across all available quarters.

# Development

## Input
Set the working directory and quarters for the run. Make sure to store all quarterly data in a subfolder called 'data' within the working directory.
...
## Setup
Load libraries, functions and set system settings.
...
In order to determine the optimal parameters for the models, multiple combinations of EMA time horizons were tested: 10-80 days for single EMA and every combination of 10-80 days constrained by fast always having to be less than slow for crossover EMA.

All positions were exited in the first 25 and last 20 minutes of the trading session and in between sessions. Any remaining missing value was populated with the previous value.
...
Single EMA momentum strategies performed poorly.
